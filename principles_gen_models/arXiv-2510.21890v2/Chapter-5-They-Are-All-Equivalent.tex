...
This chapter presents a systematic viewpoint that connects the variational-based, score-based, and flow-based perspectives within a coherent picture. While motivated by different intuitions, these approaches converge on the same core mechanism underlying modern diffusion methods. Building on \Cref{ch:variational,ch:score-based,ch:score-sde,ch:flow-based}, we observe a common recipe: define a forward corruption process that traces a path of marginals, then learn a time varying vector field that transports a simple prior to the data distribution along this path.
...
This elegant ``conditioning technique'' rewrites the objectives as expectations over the known Gaussian conditionals $p_t(\rvx_t | \rvx_0)$, yielding gradient-equivalent closed-form regression targets and tractable training objectives:
\begin{itemize}
...
The conditional versions ($\mathcal{J}_{\text{CKL}}$, $\mathcal{J}_{\text{DSM}}$, $\mathcal{J}_{\text{CFM}}$) differ from the originals ($\mathcal{J}_{\text{KL}}$, $\mathcal{J}_{\text{SM}}$, $\mathcal{J}_{\text{FM}}$) only by this shift, which leaves the gradients unchanged and thus preserves the optimization landscape. 
As a result, the minimizers remain uniquely identified with the true oracle targets, since each reduces to a least-squares regression problem whose solution recovers the corresponding conditional expectation:
\begin{mdframed}
...
\paragraph{(B) Parameterization $\mathrm{NN}_{\bm{\phi}}$ and Training Target $A_t \rvx_0 + B_t \bm{\epsilon}$.}  Users can flexibly choose the model's prediction target: the clean signal, noise, score, or velocity prediction. As detailed in \Cref{subsec:four-predictions}, all these prediction types share a common regression target of the form
\[
...
The four prediction types introduced in \Cref{subsec:four-predictions} will later be shown (\Cref{subsec:four-equiv-para}) to be equivalent under gradient minimization.  
We then broaden this view in \Cref{subsec:all-flows-are-equiv}, showing that different forward noise schedules are connected by simple time and space rescalings.
...
\end{tikzpicture}
    \caption{\textbfs{Equivalent relations among four parameterizations.} $\rvv$-prediction is given by  $\rvv^* = \alpha_t' \rvx^* + \sigma_t' \bm{\epsilon}^*$,
where clean and $\beps$-predictions are interchangeable via  
$\rvx_t = \alpha_t \rvx^* + \sigma_t \bm{\epsilon}^*$.
\figcredit{Created by the authors.}}
    \label{fig:equiv-parametrizations}
\end{figure}

\Cref{eq:predictions-equivalence} induces a one-to-one conversion (at each $t$,
given the forward noising coefficients) between the four parameterizations
\[
\bm{\epsilon}_{\bm{\phi}}(\rvx_t,t), \quad \rvx_{\bm{\phi}}(\rvx_t,t), \quad \rvs_{\bm{\phi}}(\rvx_t,t), \quad   \rvv_{\bm{\phi}}(\rvx_t,t).
\]
In practice, we train a single network in one parameterization (e.g.,
$\bm\epsilon_{\bm\phi}$). The other quantities are then \emph{defined
post hoc} by the conversions in \Cref{eq:predictions-equivalence}.
...
\end{align*}
Hence every affine path is the image of the canonical FM path under the change of variables $t\mapsto\tau(t)$ and the spatial rescaling $\rvx\mapsto c(t)\rvx$. The equality holds pointwise and therefore also in distribution.
...
Let $\rvx_t^{\mathrm{FM}}=(1-t)\rvx_0+t \beps$ and $\rvx_t=\alpha_t\rvx_0+\sigma_t\beps$ with $c(t):=\alpha_t+\sigma_t\neq 0$ and $\tau(t):=\sigma_t/(\alpha_t+\sigma_t)$. Then
...
\]
Then every affine interpolation $\rvx_t=\alpha_t\rvx_0+\sigma_t\beps$ is a rescaled and re timed trigonometric path:
...
\end{align}
The pair $(\alpha_t,\sigma_t)$ is a point in the plane. Normalizing by $R_t$ places it on the unit circle, which fixes the angle $\tau_t$ and hence the state $\rvx^{\mathrm{Trig}}_{\tau_t}$; the radius $R_t$ gives the overall scale.
...
\subsection{(Optional) Conceptual Analysis of Parametrizations and the Canonical Flow}\label{subsec:concept-why-v-affine}
...
This subsection asks a simple question: \emph{how do different parameterizations and schedules shape what the model learns and how we sample?} We proceed in two perspectives:
\begin{itemize}
  \item \textbfs{Regression Targets and Schedules.} We focus on why combining $\rvv$-prediction with the canonical linear schedule $(\alpha_t,\sigma_t)=(1-t,t)$ is natural: it maintains a stable target scale over time and eliminates curvature effects in the dynamics.
  \item \textbfs{Solver Implications.} We examine how this parameterization conceptually interacts with numerical integration schemes while deferring concrete examples such as the Euler solver and Heun’s method to \Cref{subsec:ddim-different-para,subsec:dpm-heun}.
\end{itemize}
...
Writing $\sigma_t:=\rho(t)$ and $\alpha_t:=1-\rho(t)$ for a time-varying $\rho(t)$, the conditional velocity becomes
\[
\rvv_t(\rvx_t |\rvz)
=\rho'(t)(\beps-\rvx_0),
\quad\text{where } \rvz=(\rvx_0,\beps).
...
For the canonical schedule $\rho(t)=t$, the conditional velocity $\rvv_t(\cdot |\rvz)$ satisfies
...
all else equal. With the canonical choice $\rho(t)=t$, 
...
\]
Minimizing $\mathcal K[\rho]$ therefore corresponds to finding the smoothest, least-energy
path in expectation within the affine interpolation family
\[
\rvx_t=(1-\rho(t))\rvx_0+\rho(t)\beps.
\]
With the boundary conditions $\rho(0)=0$ and $\rho(1)=1$, the
Euler--Lagrange equation $\rho''(t)=0$ gives the minimizer $\rho(t)=t$, corresponding to a
straight conditional path. This means that, among all smooth schedules $\rho$ in this family,
the canonical flow $\rho(t)=t$ is the most energy-efficient way to move from $\rvx_0$ to
$\beps$. We will revisit this point in Proposition~\ref{ot-conditional-flow}
for a more detailed treatment.


\subparagraph{Remark on the Oracle Velocity.}
If instead we evaluate the energy defined by marginal velocities
\[
\int_0^1 \E_{\rvx_t} \big[\|\rvv^*(\rvx_t, t)\|^2\big]\diff t,
\]
then with $\rvz=(\rvx_0,\beps)$ and $\rvv_t(\rvx_t |\rvz)=\rho'(t)(\beps-\rvx_0)$,
\[
\rvv^*(\rvx, t)
...
\end{align*}
where $\kappa(t):=\E_{\rvx_t\sim p_t} \left[\big\|\E[\beps-\rvx_0|\rvx_t]\big\|^2_2\right]
$.

Consequently, the \emph{marginal}-optimal schedule $\rho(t)$ need not be linear.  
It is linear iff $\kappa(t)$ is constant; in general, the Euler–Lagrange condition
\[
...
Intuitively, $\kappa(t)$ quantifies how much of the label $(\beps-\rvx_0)$ is predictable from $\rvx_t\sim p_t$:
the oracle flow slows down where $\kappa(t)$ is large, reflecting regions where the oracle velocity has high expected magnitude, and speeds up where $\kappa(t)$ is small. Hence, even though the conditional flow uses the linear schedule $(1-t,t)$,
...
The hyperparameter $\gamma(t)$ can be chosen arbitrarily, independent of $\alpha_t$ and $\sigma_t$, even after training, as it does not affect the velocity $\rvv(\rvx, t)$ or the score $\rvs(\rvx, t)$. Below are some examples:
\begin{itemize}
...
This proposition highlights a remarkable flexibility of diffusion models: once a \emph{marginal density path} $\{p_t\}_{t\in[0,T]}$ is fixed, an entire family of dynamics can reproduce it, including both the PF-ODE and the reverse-time SDEs
\[
\diff\bar\rvx(t)
