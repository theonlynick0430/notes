...
Let $\rvx_t \sim \mathcal{N}\big(\cdot; \alpha_t\rvx_0, \sigma_t^2\rmI\big)$ be a Gaussian random vector. Then Tweedie's formula says
\begin{align*}
...
\partial_t p_t(\rvx) = -\nabla_{\rvx} \cdot \bigl[ \tilde \rvf(\rvx, t) p_t(\rvx) \bigr].
\]
Both $\tilde p_t$ and $p_t$ satisfy the same continuity equation with the same initial condition $p_0$. Assuming sufficient smoothness (e.g., $\tilde \rvf \in C^1$), the solution is unique in some appropriate function space, so $\tilde p_t = p_t$. Thus, $\tilde\rvx(t) \sim p_t$ for all $t \in [0, T]$.
...
\]
with $\bar\rvx(0) \sim p_T$, where $\bar\rvw(t)$ is a standard Wiener process in reverse time, defined as $\bar\rvw(t) = \rvw(T - t) - \rvw(T)$. We need to show $\bar\rvx(t) \sim p_{T-t}$.
...
\]
The reverse-time SDE is therefore
\[
\diff \bar\rvx(t)
=
\biggl[
...
Let $s = T - t$, so $\diff t = -\diff s$. Then
\begin{align*}
\diff \bar\rvx(T - s)
&=
\biggl[
...
\end{align*}
Since $\bar\rvw(t) = \rvw(T - t) - \rvw(T)$, we have
\[
\diff \bar\rvw(T - s) = -\diff \rvw(s).
\]
Thus, defining $\bar\rvw'(s) := -\rvw(s)$, which is again a standard Wiener process, and relabeling $\bar\rvx(T-s)$ as $\bar\rvx(s)$, we obtain
\[
\diff \bar\rvx(s)
=
\biggl[
...
Let $\bar p_s(\rvx)$ denote the density of $\bar\rvx(s)$. Its Fokker-Planck equation is
\begin{align*}
...
Assume $\bar p_s = p_{T-s}$. The Fokker-Planck equation for $p_{T-s}$ from Part 1 is
\[
\partial_t p_{T-s}(\rvx)
=
-\nabla_{\rvx}\cdot\bigl[\tilde \rvf(\rvx, T - s)p_{T-s}(\rvx)\bigr].
...
Now substitute $\bar p_s = p_{T-s}$ into the Fokker-Planck equation:
\begin{align*}
\partial_s p_{T-s}(\rvx)
&=
-\nabla_{\rvx}\cdot
...
\nabla_{\rvx}\cdot\bigl[\tilde \rvf(\rvx, T - s)p_{T-s}(\rvx)\bigr].
\end{align*}
Thus, $\bar p_s = p_{T-s}$ satisfies the Fokker-Planck equation. Since $\bar\rvx(0) \sim p_T$, we have $\bar p_0 = p_T$, matching the initial condition. Uniqueness (under sufficient smoothness) ensures $\bar p_s = p_{T-s}$, so $\bar\rvx(t) \sim p_{T-t}$.
\hfill$\blacksquare$
...
\subsection{Parameter–Flow Interpretation to Proposition~\ref{vsd-grad}}
From the derivation of Proposition~\ref{vsd-grad}, we can interpret the gradient of VSD as a parameter-induced transport flow, where adjusting the model parameters moves particles in data space to align their motion with the score mismatch between the student and teacher distributions.

Let $t\sim p(t)$, $\rvz\sim p(\rvz)$, $\beps\sim\mathcal N(\bm{0},\rmI)$ and
\[
\hat\rvx_t=\alpha_t\,\rmG_\btheta(\rvz)+\sigma_t\,\beps.
\]
Define the \emph{sample (particle) velocity}
\[
\widehat{\rvv}_\btheta(\hat\rvx_t):=\partial_\btheta \hat\rvx_t=\alpha_t\,\partial_\btheta \rmG_\btheta(\rvz),
\]
and the \emph{velocity field} in $\rvx$–space as the conditional average
\[
...
Here $\rvv_\btheta(\hat\rvx_t)=\partial_\btheta \hat\rvx_t$ is the
\emph{parameter–induced particle velocity} in data space (with $t$ fixed).
Equivalently, at each fixed $t$ the density satisfies the continuity equation in $\btheta$:
\[
\partial_\btheta p_t^{\btheta}(\rvx) + \nabla_{\rvx} \cdot \big(p_t^{\btheta}(\rvx) \rvv_\btheta(\rvx)\big) = 0.
\]
...
\]
which says: adjust the parameter–flow so that particle motion aligns with the local score mismatch,
reducing the divergence along the trajectory.
...
We consider the simplified noise schedule used in EDM, where $\alpha_t = 1$ and $\sigma_t = t$, which also appears in the discussion of CTM in \Cref{sec:CTM}. Under this setting, the forward process becomes
\[
