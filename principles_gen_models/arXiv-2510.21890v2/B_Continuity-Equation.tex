...
Think of a single \nk{deterministic} update rule induced by applying a vector field (analogous to a force) 
...
If the initial states follow an underlying ``law/pattern'' described by a density $p_0$ (i.e., $\rvx_0\sim p_0$),
then applying $\bm{\Psi}$ produces a new density $p_1$ for $\rvx_1$ (i.e., $\rvx_1\sim p_1$).
...
\end{equation}
\end{mdframed}
Here $\tfrac{\partial \bm{\Psi}}{\partial \rvx}$ is the Jacobian matrix of $\bm{\Psi}$, denoted $\partial_\rvx \bm{\Psi}$.  
Equivalently, in the original coordinates,
\[
p_0(\rvx_0) \;=\; p_1\!\big(\bm{\Psi}(\rvx_0)\big)\;\big|\det \partial_\rvx \bm{\Psi}(\rvx_0)\big|.
\]
In words, $\bm{\Psi}$ reshapes the density $p_0$ into $p_1$. The factor $\big|\det \partial_\rvx \bm{\Psi}\big|$ represents the local change in volume; since probability mass is conserved, the density compensates by its inverse.  
...
\begin{nknote}
\noindent\textit{Why does the last equality hold?} It rests on two facts.
...
\[
\frac{\diff}{\diff y} f^{-1}(y) = \frac{1}{\frac{\diff}{\diff x} f(x)}.
\]
\textit{Proof.}
$\dfrac{\diff}{\diff y} f^{-1}(y) = \dfrac{\diff}{\diff y} f^{-1}(f(x)) = \dfrac{\diff x}{\diff y} = \dfrac{1}{\diff y/\diff x} = \dfrac{1}{\frac{\diff}{\diff x} f(x)}$. \hfill$\checkmark$
...
\smallskip\noindent
\textbf{Corollary.} Change the integration variable from $\rvx$ to $\rvy' = \bm{\Psi}(\rvx)$, using (A) for the volume element and (B) to rewrite the inverse Jacobian determinant:
\begin{align*}
p_{\rvy}(\rvy)
...
&= p_{\rvx}\big(\bm{\Psi}^{-1}(\rvy)\big)\,\Big|\det \frac{\partial \bm{\Psi}^{-1}}{\partial \rvy}\Big|. \qquad\blacksquare
\end{align*}
\end{nknote}
...
p_{k}(\rvx_k) \;=\; p_{k-1}(\rvx_{k-1}) \,
   ...
   \quad k=1,\dots,L.
\]
By recursion, the final density at $\rvx_L$ is
\begin{mdframed}
...
\end{align*}
This expression reflects how each transformation $\bm{\Psi}_k$ stretches or contracts volume, as captured by the Jacobian determinant. The accumulation of these local volume changes along the transformation path determines the final probability density under the composed map.
...
We now pass from discrete updates to a continuous description.  
Suppose the particle motion is driven by a time-varying velocity field 
...
In other words, $\bm{\Psi}_{0\to t}$ brings the initial state $\rvx_0$ to the solution of \Cref{eq:ode-f-continuity} at time $t$:
\[
...
\]
When $\bm{\Psi}_{0\to t}$ is smooth and invertible, this reduces to the familiar change-of-variables rule:
\[
p_t(\rvx) 
...
\]
which is actually the forward Euler discretization of the ODE in \Cref{eq:ode-f-continuity} over a small time interval $\Delta t > 0$.
...
Now, we expand the left-hand side via multivariate Taylor expansion:
\begin{align*}
    &\log p_{t+\Delta t}(\rvx_{t+\Delta t}) 
    - \log p_t(\rvx_t) 
...
\begin{align*}
    ...
    + \mathcal{O}(\Delta t^2).
\end{align*}
Matching terms with \Cref{eq:log-density-diff} and letting $\Delta t \to 0$, we conclude that
\begin{equation*}
...
\paragraph{Velocity First (Lagrangian) vs. Density First (Eulerian).}  It is important to note a key asymmetry between particle dynamics and density dynamics.  
Starting from a velocity field gives a unique flow of particles and hence a unique density evolution.  
In contrast, prescribing only the density path does not pin down a single velocity field: many different flows can lead to the same sequence of densities.
 

\subparagraph{Velocity-First (Eulerian: Flow $\Rightarrow$ Density).}  
...
\subparagraph{Density-First (Eulerian: Density $\nRightarrow$ Unique Flow).} 
...
For example, if a vector field $\mathbf w_t$ satisfies
\[
\nabla_{\rvx}\cdot\!\big(p_t(\rvx)\,\mathbf w_t(\rvx)\big)=0
\qquad\text{(no net flux w.r.t.\ $p_t$),}
\]
then both $\rvf_t$ and $\rvf_t+\mathbf w_t$ give rise to the same density evolution.  
...
We say that $p_t$ is \emph{realizable} (or \emph{generated by} $\rvf$) if there exists a velocity field $\rvf$ such that particles following
\[
\dfrac{\diff \rvx(t)}{\diff t} = \rvf(\rvx(t), t)
\]
produce exactly the densities $p_t$ through the flow map $\bm{\Psi}_{0\to t}$.  That is, realizability holds when $p_t$ and $\rvf$ together satisfy \Cref{eq:continuity-cov}.
...
\end{align*}
\end{mdframed}
Here, $ \Delta p_t = \nabla \cdot \nabla_\rvx p_t $ is the Laplacian operator.
Here, the first term describes transport of probability mass by the deterministic drift $\rvf$, while the second term models the spreading (diffusion) of the density due to stochastic noise with variance proportional to $\tfrac{1}{2} g^2(t) $.
...
Conservation implies that the rate of accumulation within $V$ equals the negative of the net outward flux:
\[
...
\begin{nknote}
\paragraph{Two Perspectives on the Divergence Theorem.}
The divergence theorem in Step~4 is the one nontrivial ingredient above. Here are two ways to see why it holds. Let $\mathbf{F} = (P, Q, R)$ be a smooth vector field on $\mathbb{R}^3$ and $\mathbf{n}$ the outward unit normal.

\subparagraph{(1) Intuition.}
...
To describe the movement of a model distribution, we first describe the movement of
its particles.  Suppose generated particles move according to a training-time
velocity field $\rvw_\tau$:
\[
...
\[
    ...
    \le 0.
\]
Therefore, Wasserstein gradient flow is gradient descent in the space of
probability distributions.  The global discrepancy $\mathcal{D}$ defines the
...
$p_{\mathrm{data}}/p_{\bphi_\tau}$, which is usually difficult to estimate for
implicit generators.  This explains why forward-KL methods are commonly
implemented through likelihoods, ELBOs, score matching, or contrastive
divergence, rather than by explicitly moving generated particles with a
Wasserstein step.
...
\[
    ...
    \right].
\]
Gradient descent therefore moves generated particles in the direction
\[
    \nabla_{\rvx}\log p_s(\hat\rvx_s)
    -
    \nabla_{\rvx}\log p_{\bphi_\tau,s}(\hat\rvx_s).
\]
This is the basic distribution-level mechanism behind DMD, VSD, and SiD:
estimate a score-mismatch direction on generated samples and use it to update
the generator.
