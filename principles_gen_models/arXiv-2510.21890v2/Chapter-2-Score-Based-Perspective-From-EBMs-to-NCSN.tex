...
can be understood directly from its two terms.
The norm term $\tfrac12\|\rvs_{\bm\phi}(\rvx)\|^2$ suppresses the score in regions where
$p_{\mathrm{data}}$ is large, making them stationary.
The divergence term $\Tr(\nabla_{\rvx} \rvs_{\bm\phi}(\rvx))$ appears with a positive sign in the loss, so minimizing the objective favors negative values of this term.
Thus, these stationary points tend to act as attractive sinks.
Together, the loss shapes high-density regions into stable and contracting points of the score field.
We explain this in detail below.

\subparagraph{Stationarity from the Magnitude Term.}
...
contribute most to the loss. The magnitude term
$\tfrac12\|\rvs_{\bm\phi}(\rvx)\|^2$ therefore drives
$\rvs_{\bm\phi}(\rvx)\to 0$ precisely in those high-probability areas, i.e., those locations become \emph{stationary}.

\subparagraph{Concavity When the Field is (Approximately) a Gradient.}
Because the divergence term $\Tr(\nabla_{\rvx} \rvs_{\bm\phi}(\rvx))$ enters the loss with a positive sign, minimizing the objective encourages the vector field to have
negative divergence in regions of high data density. Negative divergence means that nearby vectors
converge rather than spread out, so a stationary point in such a region acts as a \emph{sink}:
nearby trajectories are pulled inward. To make this precise, assume
...
Thus the learned vector field has negative divergence and the stationary point is a \emph{sink}:
small perturbations are contracted back toward $\rvx_\star$.
...
When the noise level $\sigma$ is small, the Gaussian smoothed marginal 
$p_\sigma = p_{\mathrm{data}} * \mathcal{N}(\bm{0},\sigma^2 \rmI)$, 
so their high density regions and scores nearly coincide: 
$\nabla_{\tilde{\rvx}} \log p_\sigma(\tilde{\rvx}) \approx \nabla_\rvx \log p_{\mathrm{data}}(\rvx)$. 
Consequently, taking a small step along the noisy score direction 
$\nabla_{\tilde{\rvx}} \log p_\sigma$ moves a noisy sample toward essentially the same 
high likelihood regions of the clean distribution, which is similar to the intuition 
behind score matching summarized in \Cref{subsec:training-sm}. 
By contrast, when $\sigma$ is large, the smoothing ``over simplifies'' the landscape: 
$p_\sigma$ washes out local modes and its score mostly pulls toward global mass 
(think shrinkage toward the mean), yielding coarse denoising that can over smooth. 
In practice, however, DSM typically assumes that the injected noise is small and mild.
...
\]
with $p_\sigma(\tilde{\rvx}):= \int \mathcal{N}(\tilde{\rvx};\alpha \rvx_0,\sigma^2\mathrm{I})p_{\mathrm{data}}(\rvx)\diff\rvx$ and $\tilde\rvx=\rvx+\sigma\beps$. This is exactly the Tweedie’s identity. 
Minimizing $\mathcal L_{\text{GSM}}$ with this operator trains $\rvs_\bphi$ to approximate the denoiser, recovering the denoising score matching objective.
...
\]
In the DDPM's perturbation \Cref{eq:ddpm-perturbation}  with discrete index $i$,
\[
...
\]
so minimizing \Cref{eq:ddpm-simple-loss} learns the conditional  denoiser for $\boldsymbol{\epsilon}$, which is a scaled reparameterization of the true score at noise level $i$.
...
\begin{algorithmic}[1]
\Require Trained score $\rvs_{\bm{\phi}^\times}(\cdot, \sigma_\ell)$, step sizes $\eta_\ell$, and Langevin iteration budgets $N_\ell$ for each noise level $\ell = L, \dots, 2$
\State $\rvx^{\sigma_L} \sim \mathcal{N}(\mathbf{0}, \mathbf{I})$
\For{$\ell = L, \dots, 2$}
    \State $\tilde{\rvx}_0 \gets \rvx^{\sigma_\ell}$
    \Statex\Comment{Initialize Langevin from previous noise level's output}
    ...
    \Statex \Comment{Output used as initialization for next noise level}
\EndFor
\Ensure $\rvx^{\sigma_1}$ 
\end{algorithmic}
\end{algorithm}
...
Starting from a Gaussian noise $\rvx^{\sigma_L} \sim \mathcal{N}(\bm{0}, \rmI)$, the algorithm applies Langevin dynamics at each noise level $\sigma_\ell$ to approximately sample from the perturbed distribution $p_{\sigma_\ell}(\rvx)$. The output at level $\sigma_\ell$ is used to provide a \emph{better initialization} at the next lower noise level $\sigma_{\ell-1}$.
...
\]
starting from $\tilde{\rvx}_0 := \rvx^{\sigma_\ell}$. The step size is typically scaled by the noise level:
\[
\eta_\ell = \delta \cdot \frac{\sigma_\ell^2}{\sigma_1^2}, \quad \text{for some fixed } \delta > 0.
\]
This noise-annealed refinement proceeds down to the lowest noise level $\sigma_1$, where the final sample $\rvx^{\sigma_1}$ is obtained. By progressively using the output of the previous level as better initialization for the next, this strategy enables more effective exploration and improved coverage of complex data distributions. \Cref{alg:smld} summarizes the procedure.
