...
We assume access to a finite set of samples drawn independently and identically distributed (i.i.d.) from an underlying, complex data distribution $ p_{\mathrm{data}}(\mathbf{x}) $\footnote{This is a common assumption in machine learning. For simplicity, we use the symbol $ p $ to represent either a probability distribution or its probability density/mass function, depending on the context.}.
...
When the statistical model $p_{{\bm{\phi}}^\ast}(\rvx)$ closely approximates the data distribution $p_{\mathrm{data}}(\rvx)$, it can serve as a proxy for generating new samples and evaluating probability values. This model $p_{\bm{\phi}}(\rvx)$ is commonly referred to as a \emph{generative model}.
