...
The divergence theorem is the multivariable integration by parts. For $\rmG := \phi\,\rvf p_t$, the product rule $\nabla_{\rvx}\cdot\rmG = \nabla_{\rvx}\phi^\top \rvf p_t + \phi\,\nabla_{\rvx}\cdot(\rvf p_t)$ gives
\begin{align*}
&\int \nabla_{\rvx}\phi^\top \rvf p_t\,\diff\rvx + \int \phi\,\nabla_{\rvx}\cdot(\rvf p_t)\,\diff\rvx \\
&\qquad= \int_{\mathbb{R}^D} \nabla_{\rvx}\cdot\rmG\,\diff\rvx
= \lim_{R\to\infty} \int_{\|\rvx\|=R} \rmG\cdot\mathbf{n}\,\diff S
= 0
\qquad (p_t \to 0 \text{ as } \|\rvx\|\to\infty),
\end{align*}
i.e.,
\[
\int \nabla_{\rvx} \phi^\top \rvf p_t \,\diff \rvx = -\int \phi\, \nabla_{\rvx} \cdot (\rvf p_t) \,\diff \rvx .
\]
The same identity with $\rmG = p_t\nabla_{\rvx}\phi$ and then $\rmG = \phi\nabla_{\rvx}p_t$ (using $p_t,\nabla_{\rvx}p_t \to 0$) gives
\[
