...
\begin{align*}
    ...
    &{\color{notesblue}= \frac{1}{\diff t}\,\mathbb{E}\big[\diff\big(\tilde{\rvx}_t\tilde{\rvx}_t^\top\big) \,\big|\, \rvx_0\big]} \\
    &{\color{notesblue}= \frac{1}{\diff t}\,\mathbb{E}\big[f(t)\,\tilde{\rvx}_t\tilde{\rvx}_t^\top\,\diff t + g(t)\,\diff\rvw_t\,\tilde{\rvx}_t^\top
 + f(t)\,\tilde{\rvx}_t\tilde{\rvx}_t^\top\,\diff t} \\
    &\qquad\quad{\color{notesblue}+ g(t)\,\tilde{\rvx}_t\,\diff\rvw_t^\top + g^2(t)\,\rmI_D\,\diff t + \mathcal{O}(\diff t^{3/2}) \,\big|\, \rvx_0\big]} \\
    &= \mathbb{E}[f(t) \tilde{\rvx}_t \tilde{\rvx}_t^\top + \tilde{\rvx}_t \tilde{\rvx}_t^\top f(t) + g^2(t) \rmI_D {\color{notesblue}\,|\, \rvx_0}] \\
    &= 2 f(t) \rmP(t) + g^2(t) \rmI_D.
\end{align*}
\nk{The two $\diff\rvw_t$ terms vanish because the increment $\diff\rvw_t$ is independent of $\tilde{\rvx}_t$ and of $\rvx_0$, and has zero mean.}
