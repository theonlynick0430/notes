...
\begin{align*}
    ...
    \right].
\end{align*}
Since the \nkh{second logarithmic term does not depend on $\rvx_0$}, by the \nkh{law of total probability} \nk{($\int p(\rvx_0|\rvx_i)\, p(\rvx_{i-1}|\rvx_i, \rvx_0) \,\diff\rvx_0 = p(\rvx_{i-1}|\rvx_i)$)}
\begin{align*}
    \mathbb{E}_{p(\rvx_0|\rvx_i)} \left[
    \mathbb{E}_{p(\rvx_{i-1}|\rvx_i, \rvx_0)} \left[
    \log \frac{p(\rvx_{i-1}|\rvx_i)}{p_\phi(\rvx_{i-1}|\rvx_i)}
...
The \nkh{marginal log-likelihood} is
\[
\log p_{\bm{\phi}}(\rvx_0)
=
\log \int p_{\bm{\phi}}(\rvx_0,\rvx_{1:L})\,\diff \rvx_{1:L},
\]
where the \nkh{joint generative distribution} is
\[
...
We introduce the forward noising distribution \nk{(the only difference from the HVAE is that the encoder is fixed, and it is factored slightly differently for better tractability)}
\[
p(\rvx_{1:L}| \rvx_0)
=
...
\,\diff \rvx_{1:L}.
\]
Applying \nkh{Jensen's inequality} \nk{(see \Cref{app-sec:jensen})} yields
\[
\log p_{\bm{\phi}}(\rvx_0)
\ge
...
\end{align*}
This proves the claimed decomposition. \nk{Since the encoder is fixed, only $\mathcal{L}_{\text{recon.}}$ and $\mathcal{L}_{\text{diffusion}}$ depend on $\bm{\phi}$.}
\hfill$\blacksquare$
...
\paragraph{\textit{Proof.}}
 ...
 \textbfs{Expanding $\mathcal{L}_{\text{SM}}(\bm{\phi})$. }
        Let us \nkh{expand} the \nkh{squared difference} inside the expectation:
        \begin{align*}
        ...
        \]
        \proofparagraph{\nkh{Integration by Parts.}}
        We apply the multivariate integration by parts of \Cref{app-sec:multi-ibp} to the scalar field $p_{\text{data}}$ and the vector field $\rvs_{\bm{\phi}}$, assuming that
        \[
        |s_{\bm{\phi}}^{(i)}(\rvx)\, p_{\text{data}}(\rvx)| \to 0 \quad \text{sufficiently fast as } \|\rvx\| \to \infty .
        \]
        Since $\nabla_{\rvx}\cdot\rvs_{\bm{\phi}} = \sum_{i=1}^D \partial_{x_i} s_{\bm{\phi}}^{(i)}$, we get:
        \begin{align*}
            \mathbb{E}_{\rvx \sim p_{\text{data}}(\rvx)} \left[ \langle \rvs_{\bm{\phi}}(\rvx), \rvs(\rvx) \rangle \right] 
        ...
        \end{align*}
        \begin{nknote}
        Integration by parts works here precisely because we are dealing with the score: it moves the partial derivative off the unknown $p_{\text{data}}$ and onto the known $s_{\bm{\phi}}^{(i)}$.
        \end{nknote}
        ...
        \end{align*}
        where \nkh{$C$ depends only} on the distribution \nkh{$p_{\text{data}}$}, which concludes the proof.
\hfill$\blacksquare$
...
\]
depends only on $p_{\text{data}}(\rvx)$ and $p_{\sigma}(\tilde{\rvx}|\rvx)$, and hence it is \nkh{constant with respect to $\bm{\phi}$}.
\hfill$\blacksquare$

\subsection{Lemma~\ref{tweedie}: Tweedie's Formula}\label{app-sec:tweedie}
We first state a more general form of Tweedie’s formula, which \nkh{considers time-dependent Gaussian perturbations}, and we provide its proof below.
...
The gradient of the conditional Gaussian density \nk{(note: different from the score, since there is no $\log$)} is:
\begin{align*}
...
\paragraph{Stein's Identity.} Stein’s identity is the \nkh{integration-by-parts} technique that turns expectations under an unknown density  into expectations of observable functions and their derivatives, which cancels the partition function and enables unbiased, tractable objectives and tests without ever evaluating the unknown density or the partition function. We begin with the simplest one-dimensional case and then extend it to the form needed to prove the surrogate loss for SURE.
\subparagraph{1D, Standard Normal Case.}
If $z \sim \mathcal{N}(0,1)$ and $f$ has suitable decay, then Stein's identity states:
\[
\E[f'(z)]  =  \E[z f(z)] .
\]
Denote \nkh{$\phi(z) := \tfrac{1}{\sqrt{2\pi}}  e^{-z^2/2}$}, the one-dimensional \nkh{standard normal density}.
...
- \int f(z)  \phi'(z)  \diff z.
\]
Since $\phi'(z) = -z\phi(z)$ and \nkh{$f(z)\phi(z)\to 0$ as $|z|\to\infty$ (decay condition)}, the \nkh{boundary term vanishes} and we have
\[
...
\noindent\textit{Proof.} With $\tilde{\rvx} = \rvx + \sigma\rvz$,
\begin{align*}
\E\big[(\tilde{\rvx}-\rvx)^\top \mathbf{a}(\tilde{\rvx})\,\big|\,\rvx\big]
&= \sigma\,\E\big[\rvz^\top \mathbf{a}(\rvx+\sigma\rvz)\,\big|\,\rvx\big]
\overset{\text{Stein}}{=} \sigma\,\E\big[\nabla_{\rvz}\cdot \mathbf{a}(\rvx+\sigma\rvz)\,\big|\,\rvx\big] \\
&= \sigma^2\,\E\big[\nabla_{\tilde{\rvx}}\cdot \mathbf{a}(\tilde{\rvx})\,\big|\,\rvx\big],
\end{align*}
where the middle step is \Cref{eq:stein-multi} and the last step uses $\nabla_{\rvz} = \sigma\nabla_{\tilde{\rvx}}$ (chain rule). \hfill$\blacksquare$
\endgroup
...
\begin{aligned}
&~R(\rmD;\rvx) \\
{\color{notesblue}=}&~{\color{notesblue}\E \left[\|\rmD(\tilde{\rvx})-\tilde{\rvx}+\tilde{\rvx}-\rvx\|^2  \big|  \rvx\right]} \\
...
-\underbrace{\E[(\tilde{\rvx}-\rvx)^\top \tilde{\rvx}|\rvx]}_{\substack{\nk{\sigma\,\E[\rvz]^\top\rvx \,+\, \sigma^2\E[\rvz^\top\rvz]} \\ \nk{=~} \sigma^2 D}}\Big)
\\&\qquad\qquad\qquad+\underbrace{\E[\|\tilde{\rvx}-\rvx\|^2|\rvx]}_{\substack{\nk{\sigma^2\E[\|\rvz\|^2]} \\ \nk{=~} \sigma^2 D}} \\
...
Minimizing SURE (in expectation or empirically) is thus equivalent to minimizing the true conditional MSE while \nkh{using only noisy observations}.
...
\partial_t p_t(\rvx) = -\nabla_{\rvx} \cdot \bigl[ \rvv(\rvx, t) p_t(\rvx) \bigr],
\]
\nkh{verifying the Fokker-Planck equation} in both forms.
...
\]
\nk{($\bm{\Psi}_t(\mathbf{x}_0)$ is just $\tilde\rvx(t)$ for the given initial condition $\mathbf{x}_0$.)}
Since \nkh{$\tilde\rvx(t) = \bm{\Psi}_t(\tilde\rvx(0))$}, the density $\tilde p_t(\rvx)$ of $\tilde\rvx(t)$ satisfies the \nkh{continuity equation}:
\[
...
\partial_t p_t(\rvx) = -\nabla_{\rvx} \cdot \bigl[ \rvv(\rvx, t) p_t(\rvx) \bigr].
\]
\nkh{Both $\tilde p_t$ and $p_t$ satisfy the same continuity equation with the same initial condition $p_0$}. \nkh{Assuming sufficient smoothness (e.g., $\rvv \in C^1$), the solution is unique} in some appropriate function space, \nkh{so $\tilde p_t = p_t$}. Thus, $\tilde\rvx(t) \sim p_t$ for all $t \in [0, T]$.
...
s:=T-t.
\]
\nk{(a change of variables so that both SDEs are integrated forward, from $0$ to $T$)}
Define
\[
\nkhm{\rvy(s):=\bar\rvx(T-s)},
\qquad s\in[0,T].
\]
...
\]
where \nkh{$\rvw^{\mathrm{rev}}$ is a standard Wiener process under the
forward-running clock $s$}.

Let $q_s$ denote the density of $\rvy(s)$. \nkh{Its Fokker--Planck equation is}
\begin{align*}
\partial_s q_s(\rvx)
&=
-\nabla_{\rvx}\cdot
...
\nkh{We now check that}
\[
\nkhm{q_s(\rvx)=p_{T-s}(\rvx)}
\]
...
\end{align*}
\nkh{This is exactly the Fokker--Planck equation of the reversed process}.

Finally, \nkh{$q_0=p_T$, which matches the initial distribution}
$\rvy(0)\sim p_T$. Under the regularity assumptions ensuring \nkh{uniqueness of the
