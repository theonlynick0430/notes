...
Building on this pattern, hierarchical variants (Hierarchical VAEs) stack several latent layers to capture structure at multiple scales. With this setup, Denoising Diffusion Probabilistic Models (DDPM) conceptually follow the similar template: instead of jointly training both the encoder and decoder, the encoder is fixed as a forward noising process that gradually maps data to noise, and training learns a decoder that reverses this path in successive denoising steps. In this view, VAEs, hierarchical VAEs, and diffusion models all optimize a likelihood surrogate defined by a variational bound, providing a common foundation for the methods introduced here.

\chapterlearning{
  \item understand why VAEs optimize the ELBO when exact maximum-likelihood training is difficult;
  \item see how DDPM extends the latent-variable view of VAEs into many noisy layers, with a fixed noising process and a learned reverse process;
  \item understand the conditioning trick that turns the difficult reverse-denoising problem into a tractable training target;
  \item compare mean, clean-data, and noise prediction, and see how they represent the same underlying denoising information;
  \item connect the practical DDPM denoising loss back to its variational foundation through the ELBO.
}{
Readers already familiar with VAEs and ELBO training may skim \Cref{sec:vae} and begin with \Cref{sec:ddpm}.
}
...
a \nkh{deterministic \emph{encoder}} that compresses an input to a low-dimensional latent code, and a deterministic \nkh{\emph{decoder}} that reconstructs the input from this code.  
Training minimizes the reconstruction error between the original input and its reconstruction.  
While this setup enables accurate reconstruction, the \nkh{latent space is unstructured}: \nkh{randomly sampling latent codes} usually \nkh{produces meaningless outputs}, limiting the model’s use for generation.
...
The \emph{Variational Autoencoder \nkh{(VAE)}} \citep{kingma2013auto}  solves this by \nkh{imposing} a \nkh{probabilistic structure on the latent space}. This transforms the model from a simple reconstruction tool into a true generative model, capable of producing novel and realistic data.
...
decoders, direct \nkh{MLE is computationally infeasible}, motivating the variational 
...
The ``variational'' step in VAEs addresses this by \nkh{replacing the intractable posterior with a tractable approximation}.  
We introduce an \emph{encoder} (or inference network) \nkh{$q_{\bm{\theta}}(\rvz|\rvx)$}, \nkh{parameterized by a neural network},  
...
We now define a computable training objective. While we cannot directly optimize \nkh{$\log p_{\bm{\phi}}(\mathbf{x})$}, we can \nkh{maximize} a \nkh{lower bound on it}—the \nkh{\emph{Evidence Lower Bound (ELBO)}}:
...
The ELBO \nkh{arises from Jensen's inequality} \nk{(see \Cref{app-sec:jensen})}:
\begin{align*}
...
The ELBO objective naturally decomposes into \nkh{two parts}:
\begin{itemize}
    ...
    \item \textbfs{\nkh{Reconstruction:}} Encourages accurate recovery of $\mathbf{x}$ from its latent code $\mathbf{z}$. With Gaussian encoder and decoder assumptions, this term reduces exactly to the familiar reconstruction loss of an autoencoder (cf.\ \Cref{subsec:optimization-vae}). However, as in autoencoders, optimizing this term alone risks memorizing the training data, motivating an additional regularization.
    \item \textbfs{\nkh{Latent KL:}} Encourages the encoder distribution \nkh{$q_{\bm{\theta}}(\mathbf{z}|\mathbf{x})$} to \nkh{stay close to} a simple Gaussian prior \nkh{$p_{\mathrm{prior}}(\mathbf{z})$}. This regularization shapes the latent space into a smooth and continuous structure, enabling meaningful generation by ensuring that samples drawn from the prior can be reliably decoded.
\end{itemize}
...
Recall that \nkh{maximum likelihood training amounts to} minimizing the KL divergence
\[
\mathcal{D}_{\mathrm{KL}}(p_{\text{data}}(\rvx) \| p_{\bm{\phi}}(\rvx)),
\]
which measures how well the model distribution approximates the data distribution. 
Since this term is intractable in general, the \nkh{variational framework introduces a joint comparison}.

Specifically, consider \nkh{two joint distributions}:
\begin{itemize}
...
\end{itemize}
\nkh{Comparing these distributions} yields the inequality
\begin{align}\label{eq:joint-bound}
    \mathcal{D}_{\mathrm{KL}}(p_{\text{data}}(\rvx) \| p_{\bm{\phi}}(\rvx)) 
...
\begin{align*}
    ...
    \\=&  \underbrace{\mathcal{D}_{\mathrm{KL}}(p_{\text{data}} \| p_{\bm{\phi}})}_{\text{True Modeling Error}} + \underbrace{\mathbb{E}_{p_{\text{data}}(\mathbf{x})} \big[ \mathcal{D}_{\mathrm{KL}}( q_{\bm{\theta}}(\mathbf{z}|\mathbf{x}) \| p_{\bm{\phi}}(\mathbf{z}|\mathbf{x}) ) \big]}_{\text{Inference Error}},
\end{align*}
where the first term is the true modeling error and the second is \nkh{the inference error, i.e., the gap between the approximate and true posteriors}. The latter is always non-negative, which explains \Cref{eq:joint-bound}.

Finally, note that \nk{since $p_{\bm{\phi}}(\rvx)$ does not depend on $\rvz$,}
\begingroup\color{notesblue}%
\begin{align*}
\log p_{\bm{\phi}}(\rvx)
&= \log \Big[ p_{\bm{\phi}}(\rvx) \int q_{\bm{\theta}}(\rvz|\rvx)\,\diff\rvz \Big]
= \log \int q_{\bm{\theta}}(\rvz|\rvx)\, p_{\bm{\phi}}(\rvx)\,\diff\rvz \\
&= \int q_{\bm{\theta}}(\rvz|\rvx) \log p_{\bm{\phi}}(\rvx)\,\diff\rvz
= \E_{q_{\bm{\theta}}(\rvz|\rvx)}\big[\log p_{\bm{\phi}}(\rvx)\big]
= \E_{q_{\bm{\theta}}(\rvz|\rvx)}\bigg[\log \frac{p_{\bm{\phi}}(\rvx,\rvz)}{p_{\bm{\phi}}(\rvz|\rvx)}\bigg] \\
&= \E_{q_{\bm{\theta}}(\rvz|\rvx)}\bigg[\log \bigg(\frac{p_{\bm{\phi}}(\rvx,\rvz)}{q_{\bm{\theta}}(\rvz|\rvx)} \cdot \frac{q_{\bm{\theta}}(\rvz|\rvx)}{p_{\bm{\phi}}(\rvz|\rvx)}\bigg)\bigg] \\
&= \E_{q_{\bm{\theta}}(\rvz|\rvx)}\bigg[\log \frac{p_{\bm{\phi}}(\rvx,\rvz)}{q_{\bm{\theta}}(\rvz|\rvx)}\bigg]
+ \E_{q_{\bm{\theta}}(\rvz|\rvx)}\bigg[\log \frac{q_{\bm{\theta}}(\rvz|\rvx)}{p_{\bm{\phi}}(\rvz|\rvx)}\bigg] \\
&= \mathcal{L}_{\text{ELBO}}(\bm{\theta},\bm{\phi};\rvx) + \mathcal{D}_{\mathrm{KL}}\big(q_{\bm{\theta}}(\rvz|\rvx)\,\|\,p_{\bm{\phi}}(\rvz|\rvx)\big),
\end{align*}
\endgroup
i.e.,
\[
...
\]
Thus the \nkh{inference error is exactly the gap between the log-likelihood and the ELBO}. 
\nkh{Maximizing the ELBO} therefore \nkh{corresponds to} directly \nkh{reducing inference error}, ensuring that \nkh{training minimizes} a \nkh{meaningful part} of the overall bound.
\begin{nknote}
\noindent\textit{Corollary.} Optimizing the ELBO w.r.t.\ the encoder parameters $\bm{\theta}$ corresponds to minimizing the inference error (the gap to the log-likelihood, which itself does not depend on $\bm{\theta}$); optimizing w.r.t.\ the decoder parameters $\bm{\phi}$ corresponds to minimizing the inference error while maximizing the true log-likelihood.
\end{nknote}
...
\]
where the \nkh{KL term} admits a \nkh{closed-form solution} due to the Gaussian assumption. Training the VAE therefore reduces to minimizing a regularized reconstruction loss.

\subsection{Drawbacks of Standard VAE}\label{sec:drawback_standard_VAE}
Despite the theoretical appeal of the VAE framework, it suffers from a critical \nkh{drawback}: it often produces \nkh{blurry outputs}\footnote{The limitation we study here is specific to the common VAE setup introduced
above: a simple Gaussian decoder trained through a squared reconstruction
error. In this setting, the population-optimal decoder returns a conditional
mean. If several visually different inputs correspond to overlapping latent
regions, this averaging can appear as blur. This should not be read as a
universal statement about every VAE architecture or observation model.}.
...
&\qquad\nk{= \argmin_{\bm{\mu}}\, \mathbb{E}_{q_{\text{enc}}(\rvz)} \mathbb{E}_{q_{\text{enc}}(\rvx|\rvz)} \left[ \|\rvx - \bm{\mu}(\rvz)\|^2 \right]} \\
&\qquad\nk{= \mathbb{E}_{q_{\text{enc}}(\rvz)} \Big[ \argmin_{\bm{\mu}(\rvz)}\, \mathbb{E}_{q_{\text{enc}}(\rvx|\rvz)} \left[ \|\rvx - \bm{\mu}(\rvz)\|^2 \right] \Big]}.
\end{align*}
...
\begin{align*}
\bm{\mu}^*(\mathbf{z})
&{\color{notesblue}{}= \mathbb{E}_{q_{\text{enc}}(\rvx|\rvz)}[\rvx]
= \int \rvx\, \frac{q_{\text{enc}}(\rvz|\rvx)\, p_{\text{data}}(\rvx)}{q_{\text{enc}}(\rvz)}\, \diff\rvx} \\
&=
...
\end{align*}
Now \nkh{suppose} that two \nkh{distinct inputs} $ \mathbf{x} \neq \mathbf{x}' $ are \nkh{mapped to overlapping} regions in \nkh{latent space}, i.e., the supports of \nkh{$ q_{\text{enc}}(\cdot|\mathbf{x}) $ and $ q_{\text{enc}}(\cdot|\mathbf{x}') $ intersect}. Then \nkh{$\bm{\mu}^*(\mathbf{z})$ averages} over \nkh{multiple}, potentially unrelated inputs, which leads to blurry, non-distinct outputs. This averaging effect over \nkh{conflicting modes} is a fundamental reason for the characteristic blurriness in VAE-generated samples.
...
\end{tikzpicture}
    ...
    \caption{\textbfs{Computation graph of the HVAE.} It has a hierarchical structure with stacked, trainable encoders and decoders across multiple latent layers. \nk{(Note the Markov conditioning structure.)} \figcredit{Created by the authors.}
}
...
This leads to the following top-down factorization of the \nkh{joint distribution}:
...
For encoding part, HVAEs utilize a structured, learnable \nkh{variational encoder $ q_{\bm{\theta}}(\mathbf{z}_{1:L}|\mathbf{x}) $} that mirrors the generative hierarchy. A common choice is a bottom-up Markov factorization:
\[
...
\paragraph{Why Deeper Networks in a Flat VAE are Not Enough.} There are two fundamental \nkh{limitations} of a standard flat VAE that are not resolved by simply making the encoder and decoder \nkh{deeper}.


The \nkh{first} limitation is the \nkh{variational family} \nk{(unimodal)}. In a standard VAE,
\[
...
\nkh{Second, if the decoder is too expressive}, the model may suffer from \nkh{posterior collapse}. To see why, let us recall that the objective of the VAE is
\begin{align*}
...
&\quad\quad{\color{notesblue}=\E_{p_{\text{data}}(\mathbf{x})q_\btheta(\rvz|\rvx)}[\log p_\bphi(\rvx|\rvz)]
- \int q(\rvx,\rvz)\log\frac{q_\btheta(\rvz|\rvx)}{p(\rvz)}\,\diff\rvx\,\diff\rvz} \\
&\quad\quad{\color{notesblue}=\E_{p_{\text{data}}(\mathbf{x})q_\btheta(\rvz|\rvx)}[\log p_\bphi(\rvx|\rvz)]
- \int q(\rvx,\rvz)\log\frac{q_\btheta(\rvz|\rvx)}{q_{\bm{\theta}}(\rvz)}\,\diff\rvx\,\diff\rvz} \\
&\quad\quad\phantom{=\E_{p_{\text{data}}(\mathbf{x})q_\btheta(\rvz|\rvx)}[\log p_\bphi(\rvx|\rvz)]}
{\color{notesblue}\;- \int q(\rvx,\rvz)\log\frac{q_{\bm{\theta}}(\rvz)}{p(\rvz)}\,\diff\rvx\,\diff\rvz} \\
...
\end{align*}
where $\mathcal I_q(\rvx;\rvz)$ is the \nkh{mutual information} defined by
\[
\mathcal I_q(\rvx;\rvz)
...
\]
and the \nkh{aggregated posterior} is $q_{\bm{\theta}}(\rvz)= \int p_{\mathrm{data}}(\rvx) q_\btheta(\rvz|\rvx) \diff\rvx$.

If the \nkh{decoder class can model} the \nkh{data} well \nkh{without using $\rvz$} (i.e., it contains some $p_\bphi(\rvx|\rvz)=r(\rvx)$ close to $p_{\mathrm{data}}$), then a maximizer of the \nkh{ELBO} \nkh{sets $q_\btheta(\rvz|\rvx)=p(\rvz)$}, so $\mathcal I_q(\rvx;\rvz)=0$ and $q_\btheta(\rvz)=p(\rvz)$. This ``ignore $\rvz$'' solution \nkh{does not disappear by making the networks deeper}: (1) the learned code becomes independent of $\rvx$ (so it carries no data-dependent structure useful for downstream tasks), and (2) conditioning or moving in $\rvz$ has no effect on generated samples, so controllable generation fails.
...
\nkh{Each latent level interacts only with its neighboring levels} in the
hierarchy: the encoder passes information upward through
$q_\btheta(\rvz_i|\rvz_{i-1})$, while the decoder passes information
downward through $p_\bphi(\rvz_{i-1}|\rvz_i)$. Accordingly, \nkh{the ELBO
...
While \nkh{HVAE}s extend the VAE framework with multiple latent layers for expressiveness, their \nkh{training} poses unique \nkh{challenges}. Because the \nkh{encoder} and \nkh{decoder} must be \nkh{optimized jointly}, learning becomes \nkh{unstable}: \nkh{lower layers} and the \nkh{decoder can already reconstruct $\rvx$, leaving higher-level latents with little effective signal}. Moreover, \nkh{gradient} information reaching deeper variables is often indirect and \nkh{weak}, making it difficult for them to contribute meaningfully. An additional difficulty lies in balancing model capacity, since overly expressive conditionals  can dominate the reconstruction task and suppress the utility of higher latents. 


Interestingly, the core idea of a deep, layered hierarchy finds a more powerful incarnation in variational diffusion models, a topic we explore in \Cref{sec:ddpm}. \nkh{Diffusion models} inherit the progressive structure of HVAEs but elegantly sidestep their central weakness. By \nkh{fixing} the \nkh{encoding process} and \nkh{focusing solely on learning the generative reversal}, they \nkh{unlock} newfound \nkh{stability} and modeling flexibility, leading to a significant leap in the quality of generated outputs.
...
\begin{itemize}
    \item \textbfs{The Forward Pass (Fixed Encoder):} This process gradually corrupts data by injecting Gaussian noise over multiple steps via a transition kernel \nkh{$p(\mathbf{x}_{i}|\mathbf{x}_{i-1})$}. The data evolves into an isotropic Gaussian distribution, effectively becoming pure noise. This means the encoder is \nkh{fixed} and not learned.
    \item \textbfs{The Reverse Denoising Process (Learnable Decoder):} Here, a neural network \nkh{learns} to \nkh{reverse} the noise corruption through a parameterized distribution \nkh{$p_{\bm{\phi}}(\mathbf{x}_{i-1}|\mathbf{x}_{i})$}. Starting from pure noise, this process iteratively denoises to generate realistic samples. Crucially, each individual denoising step is a more manageable task than generating a complete sample from scratch, as VAEs often attempt to do.
\end{itemize}
...
\]
Here, the process begins with \nkh{$\rvx_{0} $}, representing a sample \nkh{drawn from} the real data distribution \nkh{$p_{\text{data}}$}. The sequence $\{\beta_i\}_{i=1}^L$ denotes a pre-determined, \nkh{monotonically increasing} noise schedule, where each 
$\beta_i \in (0, 1)$ \nkh{controls the variance} of the Gaussian noise injected at step $i$.  For convenience, we \nkh{define $\alpha_i := \sqrt{1 - \beta_i^2}$}. \nk{Geometrically, $\alpha_i$ traces the quarter circle $y = \sqrt{1-x^2}$: as $\beta_i$ grows toward $1$, $\alpha_i$ shrinks to $0$.} This mathematical definition is precisely equivalent to the following intuitive iterative update:
\[
...
\nk{Why are the two forms equivalent? Conditioned on $\rvx_{i-1}$: $\E[\rvx_i|\rvx_{i-1}] = \alpha_i\rvx_{i-1} + \beta_i\,\E[\bm{\epsilon}_i] = \alpha_i\rvx_{i-1}$ and $\mathrm{Var}[\rvx_i|\rvx_{i-1}] = \beta_i^2\,\mathrm{Var}[\bm{\epsilon}_i] = \beta_i^2\rmI$, matching the kernel.}
...
\]
\begin{nknote}
\noindent\textit{Proof by induction.} The base case ($i=1$) is the transition kernel itself. For the inductive step, assume $p_i(\rvx_i|\rvx_0) = \mathcal{N}(\rvx_i; \bar{\alpha}_i\rvx_0, (1-\bar{\alpha}_i^2)\rmI)$. By the Markov chain, reparameterizing $p(\rvx_{i+1}|\rvx_i)$ and $p_i(\rvx_i|\rvx_0)$,
\begin{align*}
\rvx_{i+1}
&= \alpha_{i+1}\big(\bar{\alpha}_i\rvx_0 + \sqrt{1-\bar{\alpha}_i^2}\,\bm{\epsilon}_i\big) + \beta_{i+1}\bm{\epsilon}_{i+1}
&&(\bm{\epsilon}_i \perp \bm{\epsilon}_{i+1}) \\
&= \bar{\alpha}_{i+1}\rvx_0 + \sqrt{\alpha_{i+1}^2(1-\bar{\alpha}_i^2) + \beta_{i+1}^2}\;\bm{\epsilon}
&&\text{(sum of independent Gaussians)} \\
&= \bar{\alpha}_{i+1}\rvx_0 + \sqrt{1-\bar{\alpha}_{i+1}^2}\;\bm{\epsilon}
&&(\alpha_{i+1}^2 - \bar{\alpha}_{i+1}^2 + \beta_{i+1}^2 = 1 - \bar{\alpha}_{i+1}^2),
\end{align*}
i.e., $p_{i+1}(\rvx_{i+1}|\rvx_0) = \mathcal{N}(\rvx_{i+1}; \bar{\alpha}_{i+1}\rvx_0, (1-\bar{\alpha}_{i+1}^2)\rmI)$. \hfill$\blacksquare$
\end{nknote}
This means we can \nkh{sample $\rvx_i$ directly from $\rvx$ as}
...
\]
which motivates the choice of the \nkh{prior distribution} as
\[
...
To enable the generative process, our \nkh{goal} is to \nkh{approximate} the unknown true \nkh{reverse transition kernel, $p(\mathbf{x}_{i-1}|\mathbf{x}_i)$}. We achieve this by introducing a \nkh{learnable} parametric model, \nkh{$p_{\bm{\phi}}(\mathbf{x}_{i-1}|\mathbf{x}_i)$}, and training it to \nkh{minimize} the expected \nkh{KL divergence}:
...
\]
and analogously for $p_{i-1}(\rvx_{i-1})$. Since $p_{\mathrm{data}}$ is unknown, these integrals have \nkh{no closed-form evaluation}; at best they can be approximated from samples, so the exact densities are not available in practice.

\begin{figure}[th!]
    \centering
    \includegraphics[width=\linewidth]{Images/PartB/ddpm_conditional_trick.pdf}
    \caption{\textbfs{The conditioning trick in DDPM.}
\textbfs{(Left)}
For a fixed noisy sample $\rvx_i$, the reverse distribution
$p(\rvx_{i-1}|\rvx_i)$ averages over all possible clean samples
$\rvx_0$ that could have produced $\rvx_i$. It can therefore be a
complicated mixture.
\textbfs{(Right)}
During training, however, the clean sample $\rvx_0$ used to generate
$\rvx_i$ is known. Conditioning on this sample gives
$p(\rvx_{i-1}|\rvx_i,\rvx_0)$, a Gaussian with closed-form mean and
variance. DDPM can therefore use this tractable conditional distribution
as a simple regression target. Although each training example conditions
on only one clean sample, averaging the conditional training objective
over all samples gives the same optimal model as directly learning the
marginal reverse distribution; see Theorem~\ref{thm:equiv-marginal-kl}.
Thus, DDPM can learn the same optimal reverse prediction without directly
regressing onto the complicated, generally intractable target induced by
the marginal mixture.    \figcredit{Created by the authors with AI-assisted coding.}}
    \label{fig:conditional-trick}
\end{figure}


\paragraph{Overcoming Intractability with Conditioning.}
A central insight in DDPMs \nkh{resolves} this \nkh{intractability}: we \nkh{condition} the reverse transition \nkh{on} a clean data sample \nkh{$\mathbf{x}_0$}. This subtle yet powerful step transforms the intractable kernel into one that is \nkh{mathematically tractable}:
\[
...
\]
This tractability \nkh{arises from} two key properties of the forward process: its \nkh{Markov property}, meaning \nkh{$p(\mathbf{x}_i|\mathbf{x}_{i-1}, \mathbf{x}_0) = p(\mathbf{x}_i|\mathbf{x}_{i-1})$}, and the Gaussian nature of all involved distributions. As a result, \nkh{$p(\mathbf{x}_{i-1}|\mathbf{x}_i, \mathbf{x}_0)$ itself is Gaussian} and \nkh{admits} a \nkh{closed-form expression} (which we will see in \Cref{eq:ddpm-reverse-mean}). We visualize it in \Cref{fig:conditional-trick}. Crucially, this elegant conditioning strategy allows us to derive a tractable objective that is functionally equivalent to the seemingly intractable marginal KL divergence in \Cref{eq:marginal-kl-matching}.
...
\begin{aligned}
    ...
    \mathbb{E}_{p_i(\rvx_i|\rvx_0)} \left[
...
\end{align}
where $\bm{\mu}_{\bm{\phi}}(\cdot, i) \colon \mathbb{R}^D \to \mathbb{R}^D$ is a learnable mean function, and \nkh{$\sigma^2(i) > 0$ is fixed} as defined in \Cref{eq:ddpm-reverse-mean}.
...
\begin{align}\label{eq:ddpm-diffusion}
    \mathcal{L}_{\text{diffusion}}(\rvx_0;\bm{\phi}) := \sum_{i=2}^L \mathbb{E}_{p_i(\rvx_i|\rvx_0)}\left[\mathcal{D}_{\text{KL}}\big(p(\rvx_{i-1}|\rvx_i, \rvx_0)  \Vert  p_{\bm{\phi}}(\rvx_{i-1}|\rvx_i)\big)\right].
\end{align}
...
\begin{align}\label{eq:ddpm-diffusion-simp}
    \mathcal{L}_{\text{diffusion}}(\rvx_0;\bm{\phi}) = \sum_{i=2}^L \frac{1}{2\sigma^2(i)}  \mathbb{E}_{p_i(\rvx_i|\rvx_0)}\left\| \bm{\mu}_{\bm{\phi}}(\rvx_i, i) - \bm{\mu}(\rvx_i, \rvx_0, i) \right\|_2^2 + C,
\end{align}
where $C$ is a constant independent of $\bm{\phi}$. Averaging over the data distribution and omitting the constant $C$ (which does not affect the optimization), the final \nkh{DDPM training objective is}
\begin{align}
\begin{aligned}
    \label{eq:ddpm-mean-loss}
    \mathcal{L}_{\text{DDPM}}(\bm{\phi}) := \sum_{i=2}^L \frac{1}{2\sigma^2(i)}  \mathbb{E}_{\rvx_0} \mathbb{E}_{p_i(\rvx_i|\rvx_0)} \left[ \left\| \bm{\mu}_{\bm{\phi}}(\mathbf{x}_i, i) - \bm{\mu}(\mathbf{x}_i, \rvx_0, i) \right\|_2^2 \right],
\end{aligned}
\end{align}
where $\rvx_0\sim p_{\mathrm{data}}$.

We remark that the forward perturbation kernel $p_i(\rvx_i | \rvx_0)
$
plays an important computational role in the training objective. It gives the
distribution of the noisy state $\rvx_i$ obtained from a clean sample $\rvx_0$
after $i$ forward steps. Because each forward transition is linear and
Gaussian, their composition also has a closed-form Gaussian distribution; see
the discussion around \Cref{eq:ddpm-forward}. Consequently, after selecting a
time step $i$, we can sample $\rvx_i \sim p_i(\rvx_i | \rvx_0)
$
directly from $\rvx_0$, without first generating the intermediate states
$\rvx_1,\ldots,\rvx_{i-1}$.  Thus, each
loss term can be estimated using only a clean sample, a selected time step, and
a directly corrupted sample, making stochastic training computationally
efficient.
...
In typical DDPM implementations, training is not conducted directly using the original loss based on the \emph{mean prediction} parameterization from \Cref{eq:ddpm-mean-loss}. Instead, an \nkh{equivalent} reparameterization, known as the \nkh{\emph{$\beps$-prediction} (noise prediction)} formulation, is commonly adopted.  
...
\]
\nk{(To see this, solve \Cref{eq:ddpm-perturbation} for $\rvx_0$, substitute into \Cref{eq:ddpm-reverse-mean}, and simplify.)}
...
\]
Substituting this into the original loss leads to a squared $\ell_2$ \nkh{error between predicted and true noise}:
\[
...
Importantly, both $\mathcal{L}_{\text{DDPM}}$ and $\mathcal{L}_{\text{simple}}$ share the same optimal solution $\boldsymbol{\epsilon}^*$, this is because \Cref{eq:ddpm-simple-loss} essentially reduces to a least-squares problem (as shown similarly in Proposition~\ref{dsm-minimizer} or Proposition~\ref{equi-para})\footnote{We remark that removing the ELBO-derived weights in $\mathcal{L}_{\text{DDPM}}$ changes the training objective,
but not its population-optimal prediction: $\beps^*(\rvx_i,i)
=
\E[\beps| \rvx_i]$.
Thus, the weighted and simplified DDPM losses have the same ideal predictor, but
they emphasize different noise levels during training. In practice, this weighting
can still affect optimization and performance.}:
\[
\boldsymbol{\epsilon}^*(\rvx_i, i) = \mathbb{E}\left[ \boldsymbol{\epsilon}|\rvx_i \right], \quad \rvx_i \sim p_i.
\]
\nk{Crucially, this is only possible because the network is conditioned on the noise level $i$. The weights in $\mathcal{L}_{\text{DDPM}}$ rescale the loss at each level $i$, but since $\boldsymbol{\epsilon}_{\bm{\phi}}(\rvx_i, i)$ is free to make a separate prediction for every $i$, the minimizer at each level is the same conditional expectation $\E[\beps|\rvx_i]$ no matter how that level is weighted.}
...
\Cref{eq:ddpm-reverse-mean} motivates an alternative yet equivalent parameterization, known as \nkh{\emph{$\rvx$-prediction}} (clean prediction), in which a neural network \nkh{$\rvx_{\bm{\phi}}(\rvx_i, i)$} is \nkh{trained to predict} a \nkh{clean (denoised) sample} from a given noisy input $\rvx_i \sim p_i(\rvx_i)$ at noise level $i$. Replacing the ground-truth clean sample $\rvx_0$ in the reverse mean expression with $\rvx_{\bm{\phi}}(\rvx_i, i)$ leads to the following model:
\begin{align*}
...
The \nkh{$\rvx$-prediction} and \nkh{$\beps$-prediction} parameterizations are mathematically equivalent and \nkh{connected via the forward process}:
...
\begin{itemize}
    \item \nkh{$\mathcal{L}_{\text{prior}}$} can be made \nkh{negligible} \nkh{by choosing} the \nkh{noise schedule} $\{\beta_i\}$ such that $p(\cdot|\rvx_0) \approx p_{\text{prior}}(\cdot)$.
    \item For \nkh{$\mathcal{L}_{\text{recon.}}$}, this can be \nkh{approximated} and optimized \nkh{using a Monte Carlo estimate}; see \citep{ho2020denoising, kingma2021variational} for practical implementations. 
...
\rmkb{Diffusion's variational view fits the hierarchical latent-variable
picture in a particularly convenient way: the ``encoder'' is the fixed forward
noising chain, so there is no learned inference network. The ELBO nevertheless
does decompose into per-step KL terms, together with the terminal-prior
and reconstruction terms in \Cref{eq:ddpm-elbo}. What makes DDPM training
especially tractable is that, under the linear--Gaussian forward process, these
per-step KL terms can be rewritten as simple denoising regression problems with
closed-form conditional targets. Thus the key simplification is not the absence
of per-level KL terms, but the fact that the forward process is fixed and turns
them into supervised denoising objectives that can be sampled efficiently.}
...
More specifically, starting from a random seed $\rvx_L \sim p_{\text{prior}} = \mathcal{N}(\bm{0},\rmI)$, we recursively sample from $p_{\bm{\phi}^\times}(\rvx_{i-1} | \rvx_i)$ following the \nkh{update rule} below for $i = L, L-1, \dots, 1$:
...
at level $i-1$. This returns to the first operational sampling template introduced in
\Cref{eq:ddpm-first-try}: predict the clean part and rewrite the sample
at a lower noise level. DDPM carries out this rewrite stochastically,
combining the noise direction estimated from $\rvx_i$ with a fresh
Gaussian noise sample. A deterministic sampler follows the same basic
idea, but removes the fresh randomness and adjusts the remaining noise
coefficient accordingly. We will revisit this construction when studying
diffusion samplers more systematically in \Cref{sec:revisit-ddim}.
...
In other words, each DDPM step \nkh{\emph{predicts the clean data and
then re-noises it to exactly noise level $i{-}1$}}.
One can therefore \nkh{view DDPM sampling as iterating two operations}:
\begin{enumerate}
    \item \textbfs{Denoise.}
          \nkh{Estimate the clean data
          $\rvx_{\bm{\phi}^\times}(\rvx_i, i)$}
          from the current noisy input $\rvx_i$.
    \item \textbfs{Re-noise.}
          \nkh{Add back noise at the reduced level $i{-}1$}
...
For small forward noise scales $\beta_i$, the true reverse distribution is approximately Gaussian, enabling accurate approximation. Conversely, large $\beta_i$ induce multimodality or strong non-Gaussianity that a single Gaussian cannot capture. To maintain accuracy, DDPM employs many small $\beta_i$ steps, forming a sequential chain where each step depends on the previous and requires a neural network evaluation $\boldsymbol{\epsilon}_{\bm{\phi}^\times}(\mathbf{x}_i, i)$. This results in \nkh{$\mathcal{O}(L)$ sequential passes}, preventing parallelization and \nkh{slowing generation}.
