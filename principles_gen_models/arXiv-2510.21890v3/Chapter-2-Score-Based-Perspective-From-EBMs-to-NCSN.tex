...
In this view, points with lower energy correspond to higher probability, much like a ball rolling down into a valley. The \nkh{partition function $Z_{\bphi}$ ensures} that all \nkh{probabilities add up to one}, and \nkh{as a result only the \emph{relative} values} of energy \nkh{matter}. For instance, adding a constant to all energies multiplies both numerator and denominator by the same factor, leaving the distribution unchanged. 
...
However, in high dimensions \nkh{computing $\log Z_{\bphi}$} and its \nkh{gradient} is \nkh{intractable}, as it requires expectations under the model distribution. 
...
For a density $p(\rvx)$ on $\mathbb{R}^D$, the \nkh{\emph{score function}} is the gradient of the log-density:
\[
\rvs(\rvx) := \nabla_{\rvx} \log p(\rvx), \qquad \rvs \colon \mathbb{R}^D \to \mathbb{R}^D.
\]
Intuitively, the score forms a vector field that \nkh{points toward regions of higher probability}, providing a local guide to where the data is most likely to occur (see \Cref{fig:score-function}).
...
since $Z$ is constant in $\rvx$. This \nkh{bypasses} the \nkh{partition function} entirely.

\subparagraph{2. A Complete Representation.}  
The score function fully characterizes the underlying distribution.  
Since it is the gradient of the log-density, the \nkh{density can be recovered} (up to a constant) via
\[
\log p(\rvx) 
...
\nk{Recall the path-integral definition $\int_{\bm{\gamma}} \rvf \cdot \diff\bm{\gamma} = \int_{t_0}^{t_1} \rvf(\bm{\gamma}(t))^\top \bm{\gamma}'(t)\,\diff t$; here $\bm{\gamma}(t) = \rvx_0 + t(\rvx - \rvx_0)$, so $\bm{\gamma}'(t) = \rvx - \rvx_0$.}
...
\nkh{Sampling} from EBMs, defined by the energy function $E_{\bm{\phi}}(\mathbf{x})$, can be performed \nkh{using \emph{Langevin dynamics}}. We first present the discrete-time Langevin update and then its continuous-time limit as a stochastic differential equation (SDE). Finally, we discuss the physical intuition behind how Langevin dynamics enables efficient exploration of complex energy landscapes.
...
\end{align}
where $\mathbf{x}_0$ is initialized from some distribution (often Gaussian), $\eta > 0$ is the step size, and $\bm{\epsilon}_n \sim \mathcal{N}(\mathbf{0}, \mathbf{I})$ is Gaussian noise. The \nkh{noise enables exploration beyond local minima} by adding stochasticity. 
...
As the step size \nkh{$\eta$ approaches zero}, the \nkh{discrete} Langevin updates naturally \nkh{converge to} a continuous-time process described by the \emph{Langevin Stochastic Differential Equation \nkh{(SDE)}}\footnote{
...
Under standard regularity assumptions (e.g., $p_{\bm{\phi}}\propto e^{-E_{\bm{\phi}}}$ with a confining, sufficiently smooth $E_{\bm{\phi}}$), the distribution of \nkh{$\mathbf{x}(t)$ converges} (exponentially fast) \nkh{to $p_{\bm{\phi}}$ as $t\to\infty$}; thus we can sample by simulating (solving) the SDE \Cref{eq:continuous-langevin}. 
...
\]
which deterministically drives the particle downhill toward a local minimum of the energy function. However, such \nkh{deterministic dynamics can become trapped in local minima}, preventing exploration of the full data distribution.

To overcome this limitation, \nkh{Langevin dynamics introduces stochastic perturbations}, resulting in the SDE
\[
...
At the heart of this \nkh{inefficiency} lies the issue of poor ``mixing time'': In complex data distributions with many isolated modes, Langevin sampling often requires an extremely \nkh{long time to transition between regions of high probability}. This problem becomes significantly worse as dimensionality increases, leading to prohibitively slow convergence.
...
we \nkh{approximate} it directly as a vector field parameterized \nkh{by} a \nkh{neural network $\rvs_{\bm{\phi}}(\rvx)$} (see \Cref{fig:sm-illustration}):
\[
\rvs_{\bm{\phi}}(\rvx) \approx \rvs(\rvx).
\]
\nkh{\emph{Score matching}} fits this vector field by \nkh{minimizing} the mean squared error \nkh{(MSE)} between the \nkh{true and estimated scores}:
\begin{mdframed}
...
Fortunately, \citet{hyvarinen2005estimation} showed that \nkh{integration by parts yields} an \nkh{equivalent objective} that depends only on the model $\rvs_{\bm{\phi}}$ and the data samples, \nkh{without} requiring \nkh{access to the true score}.  
...
$\tfrac12\|\rvs_{\bm\phi}(\rvx)\|^2$ therefore \nkh{places a stronger penalty on large
...
Because the divergence term $\Tr(\nabla_{\rvx} \rvs_{\bm\phi}(\rvx))$ enters the loss with a positive sign, minimizing the objective \nkh{favors
negative divergence in regions emphasized by the data distribution}.
\nkh{Negative divergence indicates net local contraction}, although by itself it does
...
\begin{nknote}
\noindent Recall that for a vector field $\mathbf{F} = (F_1,\ldots,F_D)$,
\[
\mathrm{Div}\,\mathbf{F} := \nabla\cdot\mathbf{F}
= \begin{pmatrix} \partial/\partial x_1 \\ \vdots \\ \partial/\partial x_D \end{pmatrix} \cdot \begin{pmatrix} F_1(\rvx) \\ \vdots \\ F_D(\rvx) \end{pmatrix}
= \Tr\big(\nabla_{\rvx}\mathbf{F}\big).
\]
Intuition: if, as you take a step, $\mathbf{F}$ increases in that same direction, then the point is likely a source.
\end{nknote}
...
\paragraph{Sliced Score Matching and Hutchinson's Estimator.} Sliced score matching  replaces the trace in score matching by \nkh{averaging directional derivatives along random ``slices''}. Let $\rvu\in\R^D$ be an \emph{isotropic} random vector (e.g., Rademacher or standard Gaussian) with
$\E[\rvu]=0$ and $\E[\rvu\rvu^\top]=\mathbf I$. By \nkh{Hutchinson's identity} 
\[
\nkhm{\Tr(\rmA)=\E_{\rvu}[\rvu^\top \rmA \rvu]},\quad\text{and}\quad\E_{\rvu}[(\rvu^\top\rvs_{\bphi}(\rvx))^2]=\|\rvs_{\bphi}(\rvx)\|_2^2,
\]
...
efficiently} using JVP/VJP routines without explicit Jacobians. \nk{This works since $\rvu^\top(\nabla_{\rvx}\rvs_{\bphi})\rvu = \rvu^\top \nabla_{\rvx}(\rvs_{\bphi}^\top\rvu)$, the gradient of a scalar.}  Intuitively, this means we only check the model’s behavior along random directions: 
...
\paragraph{From Sliced to Denoising Score Matching.} \nkh{Sliced score matching} sidesteps Jacobians but still relies on the raw data distribution. 
...
and the method \nkh{only constrains} the \nkh{vector field at observed points}, 
providing \nkh{weak control in} their \nkh{neighborhoods}. 
...
To overcome the intractability of $\nabla_{\rvx} \log p_{\text{data}}(\rvx)$, \citet{vincent2011connection} proposed \nkh{injecting noise} into the data $\rvx \sim p_{\text{data}}$ \nkh{via} a known conditional distribution \nkh{$p_{\sigma}(\tilde{\rvx}|\rvx)$} with scale $\sigma$. The neural network $\rvs_{\bm{\phi}}(\tilde{\rvx}, \sigma)$ is trained to \nkh{approximate} the \nkh{score of the marginal perturbed distribution}
\[
...
Even though \nkh{$\nabla_{\tilde{\rvx}} \log p_\sigma(\tilde{\rvx})$} is \nkh{generally intractable}, \citet{vincent2011connection} showed that \nkh{\emph{conditioning} on $\rvx \sim p_{\text{data}}$} \nkh{yields} an \nkh{equivalent, tractable objective}—the \emph{Denoising Score Matching} (DSM) loss:
\begin{mdframed}
...
\]
which is also \nkh{optimal for} \Cref{eq:sm-noise-loss}.
...
Moreover, \nkh{as $\sigma \approx 0$}, $p_\sigma(\tilde{\rvx}) \approx p_{\text{data}}(\rvx)$ and
\[
\rvs^*(\tilde{\rvx}, \sigma) = \nabla_{\tilde{\rvx}} \log p_\sigma(\tilde{\rvx}) \approx \nabla_{\rvx} \log p_{\text{data}}(\rvx),
\]
indicating the \nkh{learned score approximates} the \nkh{original} data \nkh{score}, enabling its use in generation.
...
This theorem, like Theorem~\ref{thm:equiv-marginal-kl} in DDPM, illustrates a \nkh{key} shared \nkh{principle}:
\msg{Insight}{Conditioning Technique}{The conditioning technique also appears in the variational view of diffusion models in DDPM (see Theorem~\ref{thm:equiv-marginal-kl}), where \nkh{conditioning on} a data point \nkh{$\rvx$} turns an \nkh{intractable loss into} a \nkh{tractable one for Monte Carlo estimation}. A similar idea arises in the flow-based perspective (e.g., Flow Matching~\citep{lipman2022flow}), as we will see in \Cref{sec:flow-matching-framework}.}
...
\nkh{When $\sigma$ is small, the corruption is mild, so $p_\sigma$ still closely
...
regions. \nkh{As $\sigma$ becomes larger, the Gaussian smoothing progressively removes fine
...
\begin{itemize}
  \item \textbfs{Well-Defined Gradients.} The noise \nkh{perturbs data away from} its \nkh{lower-dimensional manifold}, resulting in a distribution $p_\sigma$ with full support in $\mathbb{R}^D$. Consequently, the score function $\nabla_{\tilde{\rvx}} \log p_\sigma(\tilde{\rvx})$ is well-defined everywhere.
  \item \textbfs{Improved Coverage.} The noise \nkh{smooths out sparse regions between modes}, enhancing training signal quality and facilitating Langevin dynamics to traverse low-density regions more effectively.
\end{itemize}


\subsection{Why DSM is Denoising: Tweedie's Formula}\label{subsec:why-dsm-denoising-tweedie}

We begin with \nkh{\emph{Tweedie's formula}}~\citep{efron2011tweedie}, which provides a principled basis for denoising from noisy observations alone.
Concretely, it states that: \nkh{given} a single Gaussian–\nkh{corrupted observation} $\tilde{\rvx}\sim\mathcal N(\,\cdot\,;\alpha\rvx,\sigma^2\mathrm{I})$  \nkh{from} an unknown \nkh{$\rvx \sim p_{\mathrm{data}}$}, a \nkh{denoised estimate} (the
average over all plausible clean signals given $\tilde{\rvx}$) is \nkh{obtained by nudging}
$\tilde{\rvx}$ a step of size $\sigma^2$ \nkh{in} the \nkh{direction of} the score
$\nabla_{\tilde{\rvx}}\log p_\sigma(\tilde{\rvx})$ of its \nkh{noisy marginal} defined as:
\[
...
is the denoiser.
\begin{nknote}
\noindent\textit{Proof.} In DDPM, $\rvx_i = \bar{\alpha}_i\rvx_0 + \sqrt{1-\bar{\alpha}_i^2}\,\beps$, so taking $\E[\,\cdot\,|\rvx_i]$ of both sides,
\[
\rvx_i = \E\big[\bar{\alpha}_i\rvx_0 + \sqrt{1-\bar{\alpha}_i^2}\,\beps \,\big|\, \rvx_i\big]
\;\Longleftrightarrow\;
\bar{\alpha}_i\,\E[\rvx_0|\rvx_i] = \rvx_i - \sqrt{1-\bar{\alpha}_i^2}\,\E[\beps|\rvx_i].
\]
Subtracting Tweedie's formula, $\bar{\alpha}_i\,\E[\rvx_0|\rvx_i] = \rvx_i + (1-\bar{\alpha}_i^2)\,\nabla_{\rvx_i}\log p_i(\rvx_i)$, from both sides,
\[
\E[\beps|\rvx_i] = -\sqrt{1-\bar{\alpha}_i^2}\;\nabla_{\rvx_i}\log p_i(\rvx_i). \hspace{2em}\blacksquare
\]
\end{nknote}

\paragraph{(Optional) Higher Order Tweedie's Formula.} The classical Tweedie's formula \nkh{expresses} the \nkh{posterior mean $\E[\rvx_0|\tilde{\rvx}]$} \nkh{through} the gradient $\nabla_{\tilde{\rvx}}\log p(\tilde{\rvx})$.
Higher order extensions~\citep{meng2021estimating} \nkh{express} the \nkh{posterior covariance} and higher cumulants \nkh{through higher derivatives of $\log p(\tilde{\rvx})$}.

\subparagraph{Exponential Family Setup with the Log-Normalizer $\lambda(\tilde{\rvx})$.}
Assume the conditional law of $\tilde{\rvx}$ given a latent \nkh{natural parameter} $\boldsymbol\eta\in\R^D$ belongs to a natural \nkh{exponential family} written as
\[
q_\sigma(\tilde{\rvx}|\boldsymbol\eta)
=\exp\!\big(\boldsymbol\eta^\top \tilde{\rvx}-\psi(\boldsymbol\eta)\big)\,q_0(\tilde{\rvx}).
\]
\begin{nknote}
\noindent Here $\psi(\boldsymbol\eta) := \log Z(\boldsymbol\eta)$ with $Z(\boldsymbol\eta) := \int \exp(\boldsymbol\eta^\top\tilde{\rvx})\,q_0(\tilde{\rvx})\,\diff\tilde{\rvx}$, the partition and log-partition functions. Indeed,
\[
\int q_\sigma(\tilde{\rvx}|\boldsymbol\eta)\,\diff\tilde{\rvx}
= \int \exp\big(\boldsymbol\eta^\top\tilde{\rvx} - \psi(\boldsymbol\eta)\big)\,q_0(\tilde{\rvx})\,\diff\tilde{\rvx}
= \frac{\int \exp(\boldsymbol\eta^\top\tilde{\rvx})\,q_0(\tilde{\rvx})\,\diff\tilde{\rvx}}{Z(\boldsymbol\eta)}
= 1.
\]
\end{nknote}
...
\]
which shows that, as a function of $\tilde{\rvx}$, the \nkh{posterior} has \nkh{exponential-family} form with \nkh{natural parameter $\tilde{\rvx}$}, \nkh{sufficient statistic $\boldsymbol\eta$}, and \nkh{log-partition  $\lambda(\tilde{\rvx})$}.
\begin{nknote}
\noindent\textit{Proof.} The posterior is $p(\boldsymbol\eta|\tilde{\rvx}) \propto q_\sigma(\tilde{\rvx}|\boldsymbol\eta)\,p(\boldsymbol\eta) = \exp\big(\boldsymbol\eta^\top\tilde{\rvx} - \psi(\boldsymbol\eta)\big)\,q_0(\tilde{\rvx})\,p(\boldsymbol\eta)$, whose normalizer over $\boldsymbol\eta$ is
\[
\int \exp\big(\boldsymbol\eta^\top\tilde{\rvx} - \psi(\boldsymbol\eta)\big)\,p(\boldsymbol\eta)\,\diff\boldsymbol\eta
= \frac{1}{q_0(\tilde{\rvx})}\int q_\sigma(\tilde{\rvx}|\boldsymbol\eta)\,p(\boldsymbol\eta)\,\diff\boldsymbol\eta
= \frac{p_\sigma(\tilde{\rvx})}{q_0(\tilde{\rvx})},
\]
so its logarithm is $\log p_\sigma(\tilde{\rvx}) - \log q_0(\tilde{\rvx}) = \lambda(\tilde{\rvx})$: the log-partition of the latent posterior is exactly $\lambda(\tilde{\rvx})$. \hfill$\blacksquare$
\end{nknote}
...
Second, a standard property of exponential families: the log-partition is the \nkh{cumulant generating function} of the \nkh{sufficient statistic}.
Therefore
\[
...
Specializing to the Gaussian location model with \nkh{$\boldsymbol\eta=\rvx/\sigma^2$ yields} the \nkh{familiar forms} in terms of derivatives of $\log p_\sigma(\tilde{\rvx})$:
\[
\E[\rvx|\tilde{\rvx}]
=\tilde{\rvx}+\sigma^2\nabla_{\tilde{\rvx}}\log p_\sigma(\tilde{\rvx}),
\qquad
...
In other words, SURE provides a way to select or \nkh{train denoisers when only noisy data are available}.
\nk{Main takeaway: integration by parts lets you estimate, under expectation, an MSE involving an unknown variable with an observable surrogate objective.}
...
Stein’s identity (see \Cref{app-sec:stein-identity}), however, yields the following \nkh{\emph{observable} surrogate}:
...
- D\sigma^2,
\end{align}
where $\nabla_{\tilde{\rvx}}\cdot \rmD(\tilde{\rvx})$ denotes the \nkh{divergence of $\rmD$}. 
We emphasize that $\mathrm{SURE}(\rmD;\tilde{\rvx})$ requires only the noisy observation $\tilde\rvx$, not the clean $\rvx$.

Intuitively, $\mathrm{SURE}$ consists of \nkh{two parts} that complement each other. 
The term $\|\rmD(\tilde{\rvx})-\tilde{\rvx}\|^2$ measures how far the denoiser’s output is from the noisy input; 
by itself this \nkh{underestimates} the \nkh{true error} since $\tilde{\rvx}$ is already corrupted. 
The \nkh{divergence term} acts as a \nkh{correction}: it captures how sensitive the denoiser is to small perturbations in its input, 
...
In practice, \nkh{averaging $\mathrm{SURE}$ over} both \nkh{$\rvx\sim p_{\mathrm{data}}$} and the corruption noise \nkh{$\beps$ yields} an \nkh{unbiased estimate of} the \nkh{global MSE risk}.
...
=\mathbb{E}_{\tilde{\rvx}}\!\big[\mathbb{E}_{\rvx| \tilde{\rvx}}\!\big[\|\rmD(\tilde{\rvx})-\rvx\|^2\big]\big]$ by the law of total expectation \nkh{(tower property)}. This decomposition yields a pointwise optimization: for almost every $\tilde{\rvx}$,
\[
\rmD^*(\tilde{\rvx})
=\arg\min_{\mathbf z}\ \mathbb{E}_{\rvx| \tilde{\rvx}}\!\big[\|\mathbf z-\rvx\|^2\big]
=\mathbb{E}[\rvx| \tilde{\rvx}].
\]
Therefore the \nkh{SURE-optimal denoiser coincides with} the \nkh{Bayes estimator} in \Cref{subsec:why-dsm-denoising-tweedie}, and by Tweedie’s identity:
...
\]
with a \nkh{linear operator $\mathcal L$} acting on the density. 
In the \nkh{classical case $\mathcal L=\nabla_{\rvx}$}, this gives 
\[
\nabla_{\rvx}\log p(\rvx) = \frac{\nabla_{\rvx} p(\rvx)}{p(\rvx)}
\]
The \nkh{$\frac{\mathcal L p}{p}$ structure allows integration by parts to remove normalizing constants}, yielding a tractable objective that depends only on samples from $p$ and the learned field $\rvs_\bphi$. \nk{See \Cref{app-sec:sm-trace}.}
...
To address these challenges, \citet{song2019generative} propose \nkh{injecting Gaussian noise at multiple levels} into the data distribution and jointly training a noise-conditional score network (NCSN) to estimate score functions across a range of noise scales. During generation, Langevin dynamics is applied in a noise-annealed fashion: beginning with \nkh{high-noise} levels to enable \nkh{coarse exploration}, and gradually refining toward \nkh{low-noise} levels to recover \nkh{fine details}. 
...
The goal is to train a noise-conditional score network $\rvs_{\bm{\phi}}(\rvx, \sigma)$ to estimate the score function $\nabla_{\rvx} \log p_\sigma(\rvx)$ for all $\sigma \in \{\sigma_i\}_{i=1}^L$. This is achieved by \nkh{minimizing} the DSM objective \nkh{across all noise levels}:
...
\nkh{The resulting samples are then used to initialize Langevin dynamics at the next
...
$\tilde{\rvx}_0$.
A common choice is to \nkh{scale the step size with the noise level} as
\[
\eta_\ell
=
...
\nkh{For each scale $\sigma_i$}, it performs \nkh{$K$ iterative updates} of the form ``update $\tilde{\mathbf{x}}_n$ using the score $\rvs_{\bm{\phi}^\times}(\tilde{\mathbf{x}}_n,\sigma_i)$ plus a small random perturbation'', each requiring a forward pass through the score network. Two factors necessitate large \nkh{$L \times K$}:  
\begin{enumerate}
